...
def create_app(test_config=None):
    ...
    # registering close_db and init_db_command functions from db.py
    from . import db
    db.init_app(app)

    # registering blueprint from auth.py
    from . import auth
    app.register_blueprint(auth.bp)
    
    # registering blueprint from blog.py
    from . import blog
...
# NOTES
'''
1. __name__ is the name of the current Pyhton module. The app needs to know 
   where it's located to set up some paths, and __name__ is a convenient way to
   tell it that.
   instance_relative_config=True tells the app that configuration files are
   relative to the instance folder (a place for things that change at runtime or
   configuration files). The instance folder is located outside the flaskr 
...
3. app.config.from_pyfile() overrides the default configuration with values
   ...
   - test_config can also be passed to the factory, and will be used instead
     ...
     in the tutorial can be configured independently of any development values
     you have configured.
4. os.makedirs() ensures that app.instance_path exists. Flask doesn't create the
   instance folder automatically, but it needs to be created because your
   project will create the SQLite database file there.
5. @app.route() created a simple route so you can see the application working
   before getting into the rest of the tutorial. It creates a connection between
   the URL /hello and a function that returns a response, the string in this case.
'''