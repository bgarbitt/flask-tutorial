...
arguement. The url_prefix will be prepended to all the URL's associated with the
blueprint.
'''
...
'''
What the register() view does:
1. @bp.route associates the URL '/register' with the register view function.
   When Flask reveives a request to '/auth/register', it will call the
   register view and use the return value as the response.
2. If the user submitted the form, request.method will be 'POST'. In this case,
   start validating the input.
3. request.form is a special type of dict mapping submitted from keys and
   values. The user will input their username and password.
4. Validate that username and password are not empty.
5. Validate that username is not already registered by querying the database
   and checking if a result is returned. db.execute() takes a SQL query with
   '?' placeholders for any user input, and a tuple of values to replace the
   placeholders with. The database library will take care of escaping the
   ...
   it returns None. fetchall() is used to return a list of all results.
6. If validation succeeds, insert the new user data into the database. For
   ...
   and that hash is stored. Since this query modifies data (insert), db.commit()
   needs to be called afterwards to save the changes.
7. After storing the user, they are redirected to the login page. url_for()
   generates the URL for the login view based on its name. This is preferable
   to writing the URL directly as it allows you to change the URL later without
   changing any code that links to it. redirect() generates a redirect response
   to the generate URL.
8. If validation fails, the error is shown to the user. flash() stores messages
   that can be retrieved when rendering the template.
9. When the user initially navigates to 'auth/register', or there was a
   validation error, an HTML page with the registration form shouhld be shown.
   register_template() will render a template containing the HTML, which you'll
...
'''
Some differences to register:
1. The user is queried first and stored in a variable for later use.
2. check_password_hash() hashes the submitted password in the same way as the
   stored hash and securely compares them. If they match, the password is valid.
3. session is a dict that stores data across request. When validation succeeds,
...
The url_for() function generates the view's URL based on a name and arguments.
