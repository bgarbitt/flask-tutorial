...
handling the request. Since you used an application factory, there is no appli-
cation object when writing the rest of your code. get_db will be called when
the application has been created and is handling a request, so current_app can 
be used.
...
close_db checks if a connection was created by checking if g.db was set. If the
connection exists, it is closed. Further down you will tell you application
about the close_db function in the application factory so that it is called
after each request.
'''

# Functions that will run SQL commands from schema.sql (create statements)
def init_db():
    db = get_db()
...
since we won't necessarily know where the location is when deploying the app
later. get_db returns a database connection, which is used to execute the
commands read from the file.

click.command() defines a command line command called init-db that calls the
init-db function and shows a success message to the user.
'''

# Function that takes an application and does the registration. close_db and
# init_db_command need to be registered with the app, and need this function
# because of the factory application (which is good for testing).
def init_app(app):
